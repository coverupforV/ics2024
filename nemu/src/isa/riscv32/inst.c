...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */ ) { \
  ...
  INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, (R(rd) = s->pc + 4, s->dnpc = src1 + imm), s->dnpc = s->dnpc >> 1 << 1);
  INSTPAT("??????? ????? ????? ??? ????? 11011 11", jal    , J, R(rd) = s->pc + 4, s->dnpc = s->pc + imm; IFDEF(CONFIG_FTRACE, process_jal(rd, s->pc, s->dnpc)));
  //INSTPAT("??????? ????? ????? 000 ????? 11001 11", jalr   , I, R(rd) = s->pc + 4, s->dnpc = src1 + imm, s->dnpc = s->dnpc >> 1 << 1; IFDEF(CONFIG_FTRACE, process_jalr(rd, s->isa.inst, s->pc, s->dnpc, imm)));
  // store 
